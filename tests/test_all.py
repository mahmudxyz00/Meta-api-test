...
class TestImports(unittest.TestCase):
    ...
    def test_version_is_1_2_0(self):
        import vibes_api
        self.assertEqual(vibes_api.__version__, "1.2.0")
...
class TestCookieAutoRefresh(unittest.TestCase):
    """Verify the cookie auto-refresh feature."""

    def test_get_current_cookie(self):
        c = VibesClient(meta_session="my-cookie")
        self.assertEqual(c.get_current_cookie(), "my-cookie")

    def test_auto_refresh_enabled_by_default(self):
        c = VibesClient(meta_session="x")
        self.assertTrue(c.auto_refresh)

    def test_auto_refresh_disabled(self):
        c = VibesClient(meta_session="x", auto_refresh=False)
        self.assertFalse(c.auto_refresh)

    def test_set_cookie_updates_current(self):
        c = VibesClient(meta_session="initial")
        c._set_cookie("updated")
        self.assertEqual(c.get_current_cookie(), "updated")

    def test_maybe_refresh_cookie_extracts_new_value(self):
        c = VibesClient(meta_session="old-value")

        # Simulate a response with Set-Cookie header
        class MockResp:
            headers = {"Set-Cookie": "meta_session=new-refreshed; Path=/; HttpOnly"}
            raw = None

        c._maybe_refresh_cookie(MockResp())
        self.assertEqual(c.get_current_cookie(), "new-refreshed")

    def test_maybe_refresh_cookie_no_header(self):
        c = VibesClient(meta_session="original")
        ...
        c._maybe_refresh_cookie(MockResp())
        self.assertEqual(c.get_current_cookie(), "original")

    def test_maybe_refresh_cookie_same_value(self):
        c = VibesClient(meta_session="same-value")

        class MockResp:
            headers = {"Set-Cookie": "meta_session=same-value; Path=/"}
            raw = None

        c._maybe_refresh_cookie(MockResp())
        self.assertEqual(c.get_current_cookie(), "same-value")

    def test_on_cookie_refresh_callback(self):
        saved = []
        c = VibesClient(
            meta_session="initial",
            on_cookie_refresh=lambda new: saved.append(new),
        )

        class MockResp:
            headers = {"Set-Cookie": "meta_session=callback-fired; Path=/"}
            raw = None

        c._maybe_refresh_cookie(MockResp())
        self.assertEqual(saved, ["callback-fired"])

    def test_callback_error_doesnt_break(self):
        """If the callback raises, the cookie should still update."""
        def bad_callback(new):
            raise RuntimeError("callback failed")

        c = VibesClient(
            meta_session="initial",
            on_cookie_refresh=bad_callback,
        )

        class MockResp:
            headers = {"Set-Cookie": "meta_session=new-value; Path=/"}
            raw = None

        # Should not raise
        c._maybe_refresh_cookie(MockResp())
        self.assertEqual(c.get_current_cookie(), "new-value")

    def test_auto_refresh_disabled_skips(self):
        c = VibesClient(meta_session="original", auto_refresh=False)

        class MockResp:
            headers = {"Set-Cookie": "meta_session=should-not-apply; Path=/"}
            raw = None

        c._maybe_refresh_cookie(MockResp())
        self.assertEqual(c.get_current_cookie(), "original")

    def test_background_refresh_starts(self):
        import time
        c = VibesClient(meta_session="x", background_refresh=True, refresh_interval=1.0)
        time.sleep(0.1)
        self.assertTrue(c._background_thread.is_alive())
        c.stop_background_refresh()

    def test_background_refresh_stops(self):
        import time
        c = VibesClient(meta_session="x", background_refresh=True, refresh_interval=1.0)
        time.sleep(0.1)
        self.assertTrue(c._background_thread.is_alive())
        thread = c._background_thread
        c.stop_background_refresh()
        time.sleep(0.1)
        # Thread should be None (cleared) or not alive
        self.assertTrue(
            c._background_thread is None or not c._background_thread.is_alive(),
            "Background thread should be stopped"
        )

    def test_background_refresh_idempotent(self):
        """Calling start_background_refresh twice should not create two threads."""
        c = VibesClient(meta_session="x", refresh_interval=100.0)
        c.start_background_refresh()
        t1 = c._background_thread
        c.start_background_refresh()
        t2 = c._background_thread
        self.assertIs(t1, t2)  # Same thread object
        c.stop_background_refresh()


class TestFastAPIServer(unittest.TestCase):
    """Verify the FastAPI server app builds correctly."""
    ...
    def test_app_title(self):
        from server.app import app
        self.assertEqual(app.title, "VibesAI API")

    def test_app_version(self):
        from server.app import app
        self.assertEqual(app.version, "1.2.0")
    ...
    def test_key_endpoints_exist(self):
        ...
        expected = [
            "/api/v1/me", "/api/v1/projects", "/api/v1/videos/generate",
            "/api/v1/images/generate", "/api/v1/tts", "/api/v1/voices",
            "/api/v1/media", "/api/v1/batches", "/api/v1/ingredients",
            "/api/v1/share-links", "/api/v1/timeline/chat",
            "/api/v1/publish", "/api/v1/lipsync", "/api/v1/music/search",
            "/api/v1/moodboards", "/api/v1/playables",
            "/api/v1/utils/parse-midjourney", "/api/v1/utils/validate-prompt",
            "/health", "/",
        ]
    ...
    def test_auth_dependency_exists(self):
        from server.app import verify_api_key
        self.assertTrue(callable(verify_api_key))

    def test_get_client_function_exists(self):
        from server.app import get_client
        self.assertTrue(callable(get_client))
