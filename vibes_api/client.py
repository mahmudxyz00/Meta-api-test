...
class VibesClient:
    ...
    auto_refresh : bool, default True
        **Auto-refresh the session cookie.** When True (default), the
        client intercepts ``Set-Cookie: meta_session=...`` headers from
        every API response and updates its cookie jar automatically.
        This implements the same sliding-session behavior as the Vibes
        web app, keeping your session alive indefinitely as long as you
        make at least one API call every ~30 minutes.
    background_refresh : bool, default False
        **Run a background thread that proactively refreshes the cookie.**
        When True, a daemon thread calls ``/api/auth/me`` every
        ``refresh_interval`` seconds (default 25 minutes) to keep the
        session alive even when you're not actively making API calls.
        Useful for long-running services / deployed APIs.
    refresh_interval : float, default 1500.0
        Background refresh interval in seconds (default 25 minutes).
        Vibes sessions expire after ~30 minutes of inactivity.
    on_cookie_refresh : callable, optional
        Callback ``on_cookie_refresh(new_meta_session: str)`` called
        whenever the cookie is refreshed. Useful for persisting the
        latest cookie to disk/database so you can resume later.

    Notes
    -----
    **Cookie auto-refresh** (enabled by default):

    The Vibes web app uses a sliding session — every API response
    includes a ``Set-Cookie: meta_session=...`` header with a fresh
    token that extends the session for another ~30 minutes. This
    client replicates that behavior:

    1. After every API call, the ``_check()`` method inspects the
       response's ``Set-Cookie`` header.
    2. If a new ``meta_session`` value is found, it updates the
       session's Cookie header automatically.
    3. The ``on_cookie_refresh`` callback (if set) is called with the
       new value so you can persist it.

    **Background refresh** (opt-in):

    For long-running services (e.g., a deployed FastAPI wrapper), set
    ``background_refresh=True`` to start a daemon thread that calls
    ``/api/auth/me`` every 25 minutes. This keeps the session alive
    even during idle periods.

    Example
    -------
    Basic usage (auto-refresh on every call):

    >>> client = VibesClient(meta_session="e60e910a-...-K54E")
    >>> # Cookie auto-refreshes on every API call
    >>> client.get_me()
    >>> # Get the latest cookie value (may differ from initial)
    >>> client.get_current_cookie()

    With background refresh + persistence:

    >>> def save_cookie(new_cookie):
    ...     with open(".vibes_cookie", "w") as f:
    ...         f.write(new_cookie)
    >>> client = VibesClient(
    ...     meta_session="...",
    ...     background_refresh=True,
    ...     on_cookie_refresh=save_cookie,
    ... )
    >>> # Session stays alive forever (as long as the server is up)
    """
    ...
    def __init__(
        ...
        *,
        ...
        refresh_interval: float = 1500.0,  # 25 minutes
    ...
    def get_current_cookie(self) -> str:
        """Return the current ``meta_session`` cookie value.

        This may differ from the value passed to ``__init__`` if the
        cookie was auto-refreshed. Use this to persist the latest
        cookie for later use.

        Returns
        -------
        str
            The current ``meta_session`` value.
        """
        return self._current_meta_session

    def _maybe_refresh_cookie(self, resp: requests.Response) -> None:
        """Check a response for Set-Cookie headers and update the session.

        Vibes returns a fresh ``meta_session`` cookie on every API
        response (sliding session). This method intercepts that header
        and updates the session's Cookie header automatically.
        """
        ...
        if not set_cookie:
            # Also check raw headers (some servers send multiple Set-Cookie headers)
            raw_set_cookies = resp.raw.headers.getlist("Set-Cookie") if resp.raw else []
            for raw in raw_set_cookies:
                if "meta_session=" in raw:
                    set_cookie = raw
                    break
        ...
        # Extract the meta_session value from the Set-Cookie header
        # Format: "meta_session=<value>; Path=/; HttpOnly; ..."
        for part in set_cookie.split(";"):
            part = part.strip()
            if part.startswith("meta_session="):
                new_value = part[len("meta_session="):]
                if new_value and new_value != self._current_meta_session:
                    old = self._current_meta_session
    ...
    def start_background_refresh(self) -> None:
        ...
        Calls ``/api/auth/me`` every ``refresh_interval`` seconds
        (default 25 minutes) to keep the session alive during idle
        periods. Safe to call multiple times — only one thread runs.

        The thread is a daemon, so it exits automatically when the
        main program exits.
        """
        import threading
        ...
        def _refresh_loop():
            while not self._background_stop.wait(timeout=self.refresh_interval):
                try:
                    # This call triggers _maybe_refresh_cookie via _check
                    self._get("/api/auth/me")
                except VibesAPIError:
                    # Session might be expired — can't auto-recover
                    # without user intervention
                    pass
                except Exception:
                    pass  # Network errors, etc. — retry next interval
    ...
    def _check(self, resp: requests.Response) -> dict:
        # Auto-refresh the cookie from Set-Cookie headers (sliding session)
